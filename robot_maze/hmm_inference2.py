...
def update_belief_by_evidence_log(prev_B, e, hmm):
    """Update the belief distribution over states by observation
    ...
    # Your code here
    for state in hmm.get_states():
        cur_B[state] = log(prev_B[state]) + log(hmm.pe(state, e))
    return cur_B
...
def viterbi1(prev_m, cur_e, hmm):
    """Perform a single update of the max message for modified Viterbi algorithm,
    that uses log-probabilities
    ...
    for state in hmm.get_states():
        ...
        for prev_state in hmm.get_sources(state):
            val = prev_m[prev_state] + log(hmm.pt(prev_state, state))  # fi_t-1 + log(a_ij)
            if val >= best_val:
                best_val = val
                best_state = prev_state

        cur_m[state] = log(hmm.pe(state, cur_e)) + best_val
...
def viterbi(priors, e_seq, hmm):
    """Find the most likely sequence of states using modified Viterbi algorithm,
    that uses log-probabilities
    ...
    cur_f = update_belief_by_time_step(priors, hmm)
    m = update_belief_by_evidence_log(cur_f, e_seq[0], hmm)

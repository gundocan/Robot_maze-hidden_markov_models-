...
class Robot(HMM):
    ...
    def initialize_tm(self):
        ...
        for ind, s1 in enumerate(states):
            tmp = np.array(())
            for s2 in states:
                tmp = np.append(tmp, self.pt(s1,s2))
            if ind == 0:
                tmToReturn = np.append(tmToReturn, tmp)
            else: tmToReturn = np.vstack((tmToReturn, tmp))

        self.TM = tmToReturn

    def initiliaze_em(self):
        ...
        for ind,state in enumerate(states):
            tmp = np.array(())
            for o in obs:
                tmp = np.append(tmp, self.pe(state,o))
            if ind == 0:
                emToReturn = np.append(emToReturn, tmp)
            else: emToReturn = np.vstack((emToReturn, tmp))

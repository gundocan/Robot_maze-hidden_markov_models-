...
ALL_MOVEMENTS = (ROTATE_RIGHT, MOVE, ROTATE_LEFT)
SENSOR_DIRECTIONS = (RIGHT, FORWARD, LEFT)
...
def add_sensor_dir(s, dir):
    """Add direction to a state"""
    if dir == FORWARD:  # looking forward
        return tuple([s[0] + s[2], s[1] + s[3], s[2], s[3]])
    elif dir == RIGHT:  # looking right
        if s[2:4] == NORTH:  # so right is east
            return tuple([s[0] + EAST[0], s[1] + EAST[1]]) + NORTH
        elif s[2:4] == EAST:  # so right is south
            return tuple([s[0] + SOUTH[0], s[1] + SOUTH[1]]) + EAST
        elif s[2:4] == SOUTH:  # so right is west
            return tuple([s[0] + WEST[0], s[1] + WEST[1]]) + SOUTH
        elif s[2:4] == WEST:  # so right is north
            return tuple([s[0] + NORTH[0], s[1] + NORTH[1]]) + NORTH
    elif dir == LEFT:  # looking left
        if s[2:4] == NORTH:  # so left is west
            return tuple([s[0] + WEST[0], s[1] + WEST[1]]) + NORTH
        elif s[2:4] == WEST:  # so left is south
            return tuple([s[0] + SOUTH[0], s[1] + SOUTH[1]]) + WEST
        elif s[2:4] == SOUTH:  # so left is east
            return tuple([s[0] + EAST[0], s[1] + EAST[1]]) + SOUTH
        elif s[2:4] == EAST:  # so left is north
            return tuple([s[0] + NORTH[0], s[1] + NORTH[1]]) + EAST


def add_movement(s, movement):
    """Add movement (move, turn right, turn left) to a state"""
    if movement == MOVE:
        return tuple([s[0] + s[2], s[1] + s[3], s[2], s[3]])
    elif movement == ROTATE_RIGHT:
        if s[2:4] == NORTH:
            return tuple([s[0], s[1]]) + EAST
        elif s[2:4] == EAST:
            return tuple([s[0], s[1]]) + SOUTH
        elif s[2:4] == SOUTH:
            return tuple([s[0], s[1]]) + WEST
        elif s[2:4] == WEST:
            return tuple([s[0], s[1]]) + NORTH
...
class Maze:
    ...
    def get_dist_to_wall(self, pos, dir):
        ...
        """
        if pos[0:2] not in self.get_free_positions():
            raise ValueError(
                'The specified robot position is not allowable. Did you use (row, column)?')
        d = 0
        while True:
            pos = add_sensor_dir(pos, dir)
...
class NearFarSensor:
    ...
    def get_value_probabilities(self):
        """Return the probabilities of individual observations depending on robot pose"""
    ...
    def read(self):
        """Return a single sensor reading depending on robot pose"""
...
class Robot(HMM):
    ...
    def __init__(self, sensor_directions=None, move_probs=None):
        ...
        """
        self.maze = None
        self.pose = None  # position and orientation =› tuple (x, y, orientation1, orientation2)
    ...
    def observe(self, state=None):
        ...
        :param state: robot state (pose) for which the observation
    ...
    def get_states(self):
        ...
        positions = self.maze.get_free_positions()
        states = []
        for pos in positions:
            for orient in ORIENTATIONS:
                states.append(pos + orient)
        return states
    ...
    def get_targets(self, state):
        ...
        for mov in ALL_MOVEMENTS:
            next_state = add_movement(state, mov)
    ...
    def get_next_state_distr(self, cur_state):
        ...
        p = Counter()
        for mov in ALL_MOVEMENTS:
            next_state = add_movement(cur_state, mov)
            if not self.maze.is_free(next_state):
                pass
            else:
                p[next_state] = self.move_probs[mov]
        return normalized(p)
    ...
    def pe(self, pos, obs):
        ...
        # Store current robot pose and set a new one
        stored_pos = self.pose
        self.pose = pos
        ...
        for sensor, value in zip(self.sensors, obs):
            ...
            p *= pd[value]
        # Restore robot pose
        self.pose = stored_pos
        return p

    def set_random_pose(self):
        """Set the robot to a random admissible state"""
        self.pose = random.choice(self.maze.get_free_positions()) + random.choice(ORIENTATIONS)
